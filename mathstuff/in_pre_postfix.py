...
def _is_right_associative(operator: Operator) -> bool:
	return len(operator) == 1 and operator in __right_associative_operators
...
def _pop_operator(operator_stack: OperatorStack, operator: Operator) \
	...
	if len(operator_stack):
		next_operator = operator_stack[-1]
		if next_operator == "(":
			return None
		...
		if _is_right_associative(next_operator) and \
				__precedence[next_operator] == __precedence[operator]:
			return operator_stack.pop()
